...
document.documentElement.dataset.gameVersion = "english-only-1";
...
const SPECIALS = {
  peekSelf: {
    title: "Peek Notes",
    text: "Look at one of your facedown cards."
  },
  peekOther: {
    title: "Peek Neighbor",
    text: "Look at one facedown card from another player."
  },
  passNotes: {
    title: "Pass Notes",
    text: "Swap one card with another player."
  },
  hallMonitor: {
    title: "Hall Monitor",
    text: "Flip a coin: red means caught +2 Risk, green means safe -3 Risk."
  }
};
...
const CHEATING_CHECKS = [
  {
    ...
    text: "Players with two matching number cards get +4.",
    penalty: 4,
    applies(cards) {
      const seen = new Set();
  ...
  },
  {
    ...
    text: "Players with any consecutive numbers get +5.",
    penalty: 5,
    applies(cards) {
  ...
  {
    title: "1 and 13",
    text: "Players holding both 1 and 13 get +3.",
    penalty: 3,
    applies(cards) {
      const values = new Set(numericValues(cards));
  ...
  {
    title: "Three Odd Cards",
    text: "Players with at least three odd cards get +4.",
    penalty: 4,
    applies(cards) {
  ...
  },
  {
    ...
    text: "Players holding 5 or 10 get +3.",
    penalty: 3,
    applies(cards) {
...
function startGame() {
  ...
  addLog("Exam begins. You secretly looked at your first two cards.", {
    type: "start",
    actor: 0
  });
  render();
...
function estimatePlayerLabel(playerIndex) {
  ...
  if (playerIndex === 0) return `Known Risk: ${knownHumanScore() + player.modifier}`;
  const knownCount = player.known.filter(Boolean).length;
  return knownCount ? `${knownCount} seen` : "Unknown";
}
...
function renderCheckRule() {
  ...
  penalty.textContent = `Hit +${check.penalty} Risk`;
  ...
  state.players.slice(1).forEach((player, offset) => {
    ...
    player.cards.forEach((card, cardIndex) => {
      ...
      const element = renderCard(card, {
        ...
        small: true,
        badge: player.known[cardIndex] && !state.gameOver ? "Known" : "",
        onClick: selectable
...
function renderPiles() {
  ...
  if (top) {
    ...
    els.discardTop.textContent = "Empty";
  }
...
function renderHuman() {
  ...
  state.players[0].cards.forEach((card, index) => {
    ...
    const element = renderCard(card, {
      ...
      badge: state.players[0].known[index] && !state.gameOver ? "Known" : "",
      onClick: selectable
...
function renderTurnPanel() {
  ...
  if (state.gameOver) {
    els.phasePill.textContent = "Scored";
    els.instruction.textContent = "This game is over.";
    return;
  }

  if (state.currentPlayer !== 0) {
    els.phasePill.textContent = `${currentPlayer().name} Acting`;
    els.instruction.textContent = "AI is thinking.";
    els.drawnArea.appendChild(note("Please wait. The AI players will finish their turns automatically."));
    return;
  }

  if (state.doneDeclaredBy !== null) {
    els.phasePill.textContent = `Final ${state.finalTurnsRemaining} turns`;
  } else {
    els.phasePill.textContent = "Your Turn";
  }
  ...
  if (state.selection) {
    ...
    if (state.drawnCard) {
      ...
      label.textContent = "Click a glowing hand card on the table.";
      els.drawnArea.appendChild(label);
    } else {
      els.drawnArea.appendChild(note("Click a glowing hand card on the table."));
    }
    addActionButton("Cancel", "ghost", cancelBoardSelection);
    return;
  }

  if (state.phase === "chooseSource") {
    els.instruction.textContent = "Choose the draw pile, or take the top card from the discard pile.";
    els.drawnArea.appendChild(note("The discard pile is public. The draw pile is unknown."));
    return;
  }

  if (state.phase === "action" || state.phase === "chooseSwap") {
    els.instruction.textContent = state.phase === "chooseSwap"
      ? "Choose one of your cards to replace. The old card goes to the discard pile."
      : "Decide what to do with the card you just took.";
    els.drawnArea.appendChild(renderCard(state.drawnCard, { visible: true, medium: true }));
    const label = document.createElement("div");
    label.className = "drawn-note";
    ...
    if (humanSkillInProgress) {
      els.instruction.textContent = "The Hall Monitor is approaching. Wait for the check.";
      els.drawnArea.appendChild(note("Resolving the skill. The coin window will appear soon."));
      return;
    }

    if (state.drawnCard.kind === "number") {
      addActionButton("Swap In", "primary", () => {
      ...
      addActionButton("Use Special", "gold", useHumanSpecial);
    }
    addActionButton("Discard", "ghost", discardHumanDrawn);
    return;
  }

  if (state.phase === "doneDecision") {
    els.instruction.textContent = "Your turn is over. Continue, or declare Done so every other player gets one final turn.";
    els.drawnArea.appendChild(note("Timing matters: the lowest Risk Score wins, but a random Cheating Check happens at the end."));
    addActionButton("Continue", "primary", () => finishHumanTurn(false));
    addActionButton("Declare Done", "gold", () => finishHumanTurn(true));
  }
}
...
function renderCard(card, options = {}) {
  ...
  if (isValueCard(card)) {
    ...
    img.alt = card.kind === "skill"
      ? `Skill Card ${card.value}`
      : `Number Card ${card.value}`;
...
function isTeacherReactionButton(label) {
  return label.includes("Use Skill")
...
function showKnownCardPreview(playerIndex, cardIndex) {
  ...
  return showCardPreview({
    title: `${player.name} Slot ${cardIndex + 1}`,
...
function cardShortName(card) {
  ...
  return SPECIALS[card.type].title;
}

function cardDescription(card) {
  if (card.kind === "number") return `Number Card ${card.value}. It counts toward your final Risk Score.`;
  if (card.kind === "skill") {
    const special = SPECIALS[card.type];
    return `Skill Card ${card.value}: ${special.title}. ${special.text} If kept in hand, it still counts as ${card.value} Risk.`;
  }
  const special = SPECIALS[card.type];
  return `${special.title}: ${special.text}`;
}
...
function refillDeckIfNeeded() {
  ...
  addLog("The draw pile ran out. The discard pile was shuffled into a new draw pile.", {
    type: "reshuffle"
  });
}
...
function humanDrawFromDeck() {
  ...
  state.phase = "action";
  addLog(`You drew ${cardShortName(card)} from the draw pile.`, {
    type: "draw",
    actor: 0,
    source: "draw",
    card,
...
function humanTakeDiscard() {
  ...
  state.drawnCard = card;
  state.phase = "action";
  addLog(`You took ${cardShortName(card)} from the discard pile.`, {
    type: "draw",
    actor: 0,
    source: "discard",
    card,
...
function swapHumanCard(index) {
  ...
  addLog(`You swapped ${state.drawnCard.value} into slot ${index + 1} and discarded ${cardShortName(oldCard)}.`, {
    type: "swapIn",
    actor: 0,
...
function discardHumanDrawn() {
  ...
  addLog(`You discarded ${cardShortName(state.drawnCard)}.`, {
    type: "discard",
    actor: 0,
    card: state.drawnCard,
    visible: true
...
async function useHumanSpecial() {
  ...
  if (card.type === "peekSelf") {
    const pick = await beginBoardCardSelection({
      title: "Peek Notes",
      instruction: "Click one of your own hand cards to look at it.",
    ...
    await showCardPreview({
      title: `Your Slot ${pick.cardIndex + 1}`,
      text: "You now remember this card.",
      card: state.players[0].cards[pick.cardIndex]
    });
    addLog(`You looked at and remembered your card in slot ${pick.cardIndex + 1}.`, {
      type: "peek",
      actor: 0,
      target: 0,
      card: state.players[0].cards[pick.cardIndex],
  ...
  if (card.type === "peekOther") {
    const pick = await beginBoardCardSelection({
      title: "Peek Neighbor",
      instruction: "Click one hand card from any opponent to look at it.",
      allow: playerIndex => playerIndex !== 0
    });
    if (!pick) return;
    ...
    render();
    await showCardPreview({
      title: `${state.players[target].name} Slot ${index + 1}`,
      text: "You saw this opponent card.",
      card: state.players[target].cards[index]
    });
    addLog(`You saw ${state.players[target].name}'s hand card in slot ${index + 1}.`, {
      type: "peek",
      actor: 0,
      target,
      card: state.players[target].cards[index],
  ...
  if (card.type === "passNotes") {
    const ownPick = await beginBoardCardSelection({
      title: "Pass Notes",
      instruction: "First click one of your hand cards to give away.",
      allow: playerIndex => playerIndex === 0
    });
    if (!ownPick) return;
    const targetPick = await beginBoardCardSelection({
      title: "Pass Notes",
      instruction: "Then click one opponent hand card to finish the swap.",
      allow: playerIndex => playerIndex !== 0
    });
    ...
    addLog(`You swapped one card with ${state.players[target].name}.`, {
      type: "pass",
      actor: 0,
      target
    });
...
function showHallMonitorCoinFlip(playerIndex) {
  return new Promise(resolve => {
    ...
    const flipCoin = () => {
      ...
      els.coinResult.textContent = "The coin is flipping.";
      ...
      els.coinResult.textContent = outcome
          ? "Red: caught by the Hall Monitor, +2 Risk."
          : "Green: safe, -3 Risk.";
    ...
    const player = state.players[playerIndex];
    els.coinIntro.textContent = `${player.name} flips a coin: red is caught +2, green is safe -3.`;
    els.coin.className = "coin waiting";
    els.coinResult.className = "coin-result";
    els.coinResult.textContent = "Flip the coin to decide.";
...
function resolveHallMonitor(playerIndex, forcedCaught = null) {
  ...
  if (caught) {
    player.modifier += 2;
    addLog(`${player.name} got caught by the Hall Monitor: +2 Risk.`, {
      type: "monitor",
      actor: playerIndex,
      outcome: "caught"
    });
    ...
    player.modifier -= 3;
    addLog(`${player.name} avoided getting caught: -3 Risk.`, {
      type: "monitor",
      actor: playerIndex,
      outcome: "clear"
    });
...
async function chooseCardSlot(title, text, playerIndex) {
  ...
  return showChoice(title, text, player.cards.map((card, index) => ({
    label: playerIndex === 0 && player.known[index]
      ? `Slot ${index + 1}: ${cardShortName(card)}`
      : `Slot ${index + 1}`,
    value: index
  })));
}
...
function finishHumanTurn(declareDone) {
  if (declareDone && state.doneDeclaredBy === null) {
    ...
    addLog("You declared Done. Every other player gets one final turn.", {
      type: "done",
      actor: 0
    });
  }
...
function runAiTurn() {
  ...
  addLog(`${player.name} took ${cardShortName(card)} from the ${sourceEn}.`, {
    type: "draw",
    actor: index,
  ...
  if (state.doneDeclaredBy === null && shouldAiDeclareDone(index)) {
    ...
    state.finalTurnsRemaining = state.players.length - 1;
    addLog(`${player.name} declared Done. Every other player gets one final turn.`, {
      type: "done",
      actor: index
    });
  }
...
function aiHandleNumber(playerIndex, card) {
  ...
  if (highestIndex !== -1 && card.value < player.cards[highestIndex].value) {
    ...
    state.discardPile.push(oldCard);
    addLog(`${player.name} swapped out a high-risk card.`, {
      type: "swapIn",
      actor: playerIndex,
      card,
      oldCard,
    ...
    state.discardPile.push(card);
    addLog(`${player.name} discarded ${cardShortName(card)}.`, {
      type: "discard",
      actor: playerIndex,
      card,
      visible: false
...
function aiUseSpecial(playerIndex, card) {
  const player = state.players[playerIndex];
  if (card.type === "peekSelf") {
    addLog(`${player.name} peeked at their own notes.`, {
      type: "peek",
      actor: playerIndex,
      target: playerIndex,
      visible: false
  ...
  if (card.type === "peekOther") {
    ...
    addLog(`${player.name} peeked at one card from ${state.players[target].name}.`, {
      type: "peek",
      actor: playerIndex,
      target,
      visible: false
  ...
  if (card.type === "passNotes") {
    ...
    if (ownIndex !== -1) {
      ...
      addLog(`${player.name} passed notes with ${state.players[target].name}.`, {
        type: "pass",
        actor: playerIndex,
        target
      });
...
function finishGame() {
  ...
  addLog(`Cheating Check revealed: ${check.title}. Lowest Risk Score wins.`, {
    type: "check"
  });
  render();
...
function showScores(check, rows, winnerScore) {
  ...
  els.scoreTitle.textContent = "Card Scores";
  els.finalCheck.textContent = "First, check each player's four-card score.";
...
function renderScoreRow(row, options = {}) {
  ...
  if (options.stage === "final") {
    ...
    detail.textContent = `Cards ${row.base} + Monitor ${formatSigned(row.modifier)} + Check ${formatSigned(row.penalty)} = ${row.total}`;
...
function revealFinalScores() {
  ...
  els.scoreTitle.textContent = "Final Reckoning";
...
function showVictory(winner) {
  ...
  playVictorySound();
  els.finalCheck.textContent = `${winner.player.name} remains on the board. Lowest Risk wins.`;
